...
ATOM winclass() // Регистрация класса окна
{
    ...
    wc.lpfnWndProc = (WNDPROC)WinProc; // Адрес оконной функции
...
ATOM childwinclass() // Регистрация класса окна
{
    ...
    wc.cbClsExtra = 0; // Дополнительная память
    wc.cbWndExtra = 0; // Дополнительная память
    wc.hInstance = childHin; // Экземпляр приложения
    wc.hIcon = LoadIcon(childHin, IDI_APPLICATION); // Пиктограмма для окон
    wc.hCursor = LoadCursor(childHin, IDC_ARROW); // Курсор мыши для окон
    wc.hCursor = LoadCursor(childHin, IDC_APPSTARTING); // Курсор мыши для окон
    wc.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH); // Кисть для окон
    wc.lpszMenuName = NULL; // Ресурс меню окон
    wc.lpszClassName = winChClassName; // Имя класса окон
    wc.hIconSm = LoadCursor(childHin, IDC_APPSTARTING); // иконка
    return RegisterClassEx(&wc); // Pегистрация класса окна.
}
